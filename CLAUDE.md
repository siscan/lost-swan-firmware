...
data/                          ring.json (generated by tools/ringgen.py) → LittleFS upload
web/sim/                       browser simulator - replays recorded real-logic traces
web/                           static UI sources → LittleFS image (Phase 3)
...
**Phase 2 done (fluid ring, frame, modes, time, simulator). Next: Phase 3**
per spec §15 — web UI (Terminal, Modes, Calibrate, Settings, Diagnostics,
Update) on esp_http_server + WebSocket, reusing the §10.2a dispatcher in
ModeManager; the sim page's MockSocket already speaks the intended /ws shape.
WiFi itself arrives in Phase 4 — until then SNTP never syncs and the clock
shows the WiFi glyph. Motion changes must keep `docs/MOTION_SYNC.md` true
(including the seqlock rule: no consumer reads two relaxed atomics as a pair
outside axis_read_published) and the simulated-axis suite green; Linux CI is
the reliability source of truth. Nothing has run on hardware yet — the board
has not arrived; the gear ratio (85/33 vs the stale 68/26 prose) is settled
by bench step 4, not by code.
