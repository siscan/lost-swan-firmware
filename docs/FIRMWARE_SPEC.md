...
3. `step 0 200` — confirm the drum turns in the **descending** direction: one
   ...
   **`dir 1` then `save`** — DIR is the ganged GPIO24 and `motion.dir_invert`
   settles it in firmware (§2.2, since 2026-09-06). Ganged: all five or none,
   and refused while a column is MOVING. Coil order on the JSTs does not need
   to match.
   *(This said "move that driver's DIR tie to the other rail" until 2026-09-11 —
   correct for the rim-gear bridge, where DIR was tied per driver and no pin
   could be spared. It remains correct on the **XIAO**: `PIN_DIR = -1` there,
   so direction is a wiring change on that board.)*
